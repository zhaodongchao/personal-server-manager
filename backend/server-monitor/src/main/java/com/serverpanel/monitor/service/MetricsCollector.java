...
public class MetricsCollector {
    ...
    /** 采集 + 缓存 + 广播 */
    @Scheduled(fixedDelayString = "PT${serverpanel.monitor.interval-seconds:5}S",
               initialDelayString = "PT2S")
    public void collect() {
        try {
    ...
    public MonitorOverview overview() {
        ...
        for (OSFileStore store : os.getFileSystem().getFileStores(true)) {
            ...
            disk.setUsage(total == 0 ? 0
                : round2((total - store.getUsableSpace()) * 100.0 / total));
            disks.add(disk);
        }
        vo.setDisks(disks);
    ...
    public List<MetricFrame> history(Duration duration) {
        List<String> raw = redisTemplate.opsForList()
            .range(CacheConstants.MON_FRAME_RECENT, 0, -1);
    ...
    public void hourlyAggregate() {
        try {
            ...
            int n = frames.size();
            double cpu = frames.stream().mapToDouble(MetricFrame::getCpuUsage).average().orElse(0);
            double mem = frames.stream().mapToDouble(MetricFrame::getMemUsage).average().orElse(0);
            double disk = frames.stream().mapToDouble(MetricFrame::getDiskUsage).average().orElse(0);
            double load = frames.stream().mapToDouble(MetricFrame::getLoadAvg1).average().orElse(0);
            double netInMb = frames.stream().mapToDouble(MetricFrame::getNetInRate).sum() * 5 / 1024;
            double netOutMb = frames.stream().mapToDouble(MetricFrame::getNetOutRate).sum() * 5 / 1024;
