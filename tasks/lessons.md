...
## 2026-09-22 服务器配置管理（非侵入式 drop-in + 一键生效）

### 12. 宿主 op 必须注册在 `__main__` 守卫之前，且必须「真实调用」验证
- **现象**：新增 `sys.*` 七个 op 追加到 `hostagent.py` 末尾，`grep -n "@op('sys.detect')"` 有输出、
  代码也在文件里，但调用一律 `unknown-op`。
- **根因**：`if __name__ == '__main__': serve()` 位于文件中**部**；`serve()` 是阻塞循环，
  **其后追加的代码永不执行**，装饰器从未运行 → op 未注册。`grep` 只能证明「文本在」，
  不能证明「注册了」，属典型的假阳性验证。
- **修正**：把新 op 块插到 `if __name__ == '__main__'` **之前**；仓库内
  `psm/up/rebuild_hostagent.py` 做幂等重建（先剥离旧 sys 段，再插到守卫前）。
- **教训**：**凡是「注册/加载」类改动，验证必须走真实调用**（`host.ping` → 目标 op），
  文本存在性检查不作数。

### 13. drop-in 目录「不存在」不等于「不可用」——可用性门槛看主配置 Include
- **现象**：SSH 类别在页面上整类灰显「不可用」，原因写的是「目录不存在: /etc/ssh/sshd_config.d」。
- **事实**：`/etc/ssh/sshd_config` 第 12 行**确实**有 `Include /etc/ssh/sshd_config.d/*.conf`，
  只是该目录在 Debian 12 上默认不存在 —— 而本模块的写入阶段本来就会 `os.makedirs` 按需建目录。
- **根因**：可用性判定门槛选错了：拿「目录是否已存在」当门槛，把「可写」误判成「不可用」。
- **修正**：改为解析主配置里**未被注释**的 `Include` 指令，看目标目录是否在其中；
  只有「主配置根本没 Include」才算不可用（这种情况写进去不会生效），且原因文案要说清
  「需先手工启用该 Include」。
- **教训**：**可用性判据要盯「真正的前置条件」**，而不是「当前文件系统长什么样」；
  能被自己按需创建的目录，永远不该成为门槛。

### 14. 「实测值 == 托管值」不能证明配置写入生效
- **现象**：sshd 回读 `UseDNS=no` 与托管值一致，看似生效；停止托管后仍是 `no`，
  一度被当成「回滚没生效」的缺陷。
- **真相**：Debian 这份 OpenSSH 的 `sshd_config(5)` 写明 `UseDNS` **默认值就是 `no`**
  （`If this option is set to no (the default)`）。默认值恰好等于托管值，因此它**没有区分度**。
- **修正**：回读对照必须选**与发行版默认不同**的项。本机用 `MaxAuthTries`（3 vs 默认 6）、
  `ClientAliveInterval`（300 vs 默认 0）、`ClientAliveCountMax`（2 vs 默认 3）三项，
  三者在「停止托管」后都回到了默认值，才是「回到发行版原状」的有效证据。
- **教训**：**验证要用有区分度的样本**；断言前先确认「默认值是什么」，否则容易把
  「本来就相等」误读成「生效成功」，或把「本来就相等」误报成「回滚失败」。

### 15. 「停止托管」必须失败安全：删前留内容，重载失败要写回
- **风险**：最初的 `sys.unmanage` 先把托管片段**和全部备份**一起删掉，再去重新生效。
  若这一步失败（如 `systemctl restart sshd` 出错），就留下「片段已删 + 备份已删 + 服务仍跑旧配置」
  的半残状态，且**已无内容可回退**。
- **修正**：删除前把片段内容读进内存；重新生效失败时写回原内容并再跑一次生效，
  同时返回 `unmanaged: false` 让后端记一条 FAILED（而不是假成功）。
- **教训**：**破坏性操作的前置备份要等到「新状态确认可用」之后才能丢**；
  先删后建时，删除动作本身也必须可逆。

### 16. JSDoc 里写 `/etc/*/x.conf` 会提前闭合块注释
- **现象**：前端 `pnpm build:antd` 直接失败，报 Babel 解析错误，指向 `index.vue` 的文档注释行。
- **根因**：注释里写了 `{@code /etc/*/99-serverpanel.conf}`，其中的 `*` + `/` 构成 `*/`，
  **块注释提前结束**，后面的中文变成代码。
- **修正**：改成不含 `*/` 的写法（如「各类别配置目录下的 `99-serverpanel.conf`」）。
- **教训**：`/** */` 注释里出现 `/etc/*/`、`a*/b` 这类片段会静默截断注释；
  通配符路径要么改写、要么用反引号包住但**仍要避开 `*/`**。

### 17. CDP 探针的两个小坑：漏传 sessionId 与折叠菜单用错属性
- **坑 A**：`Runtime.evaluate` 报 `-32601 'Runtime.evaluate' wasn't found`。
  真因是**漏传 `sessionId`**，命令打到了浏览器级目标（Browser 域没有 Runtime）。
  修法：`evalJs` 内部对 `sid || SESSION` 做兜底，并在报错里带上方法名与是否带 session。
  另外 SPA 登录跳转瞬间执行上下文会短暂不可用，`eval` 需带重试。
- **坑 B**：断言「侧边菜单出现 X」用 `document.body.innerText` 失败，但页面其实是对的 ——
  刚登录落在 `#/overview/workspace` 时「运维工具」处于**折叠态**，`innerText` 只有可见文本，
  取不到子项；`aside.textContent` 才能取到。**折叠/虚拟滚动的菜单不要用 innerText 断言**，
  更稳的做法是再加一条数据级取证（`/menu/all` 响应体里是否含该路由节点）。
