...
<template>
  ...
  >
    <Alert class="mb-4" show-icon type="warning">
      <template #message>
                第一类（自增计数器 / 序列）已改为 <b>真连登记库取号</b>：先在「应用栈 → 数据库 → 取号数据源」登记
        PostgreSQL / MySQL 实例，本页选对应方案后，每个号都由目标库的 InnoDB 自增计数器或数据库序列
        <b>实际分配</b>（可观察真实空洞 / 跳号）；本页不落业务表。
        本页生成的 ID 不参与任何业务，仅供观察与对照。
      </template>
    </Alert>
