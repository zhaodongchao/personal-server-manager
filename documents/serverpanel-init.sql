-- =============================================================================
-- ServerPanel MySQL 初始化脚本（独立可执行版）
-- 适用：全新服务器手动初始化数据库；与 Flyway 迁移脚本(V1__init.sql)等效，
--       执行本脚本后首次启动应用，Flyway 将自动跳过已存在的结构。
-- 用法：
--   mysql -uroot -p < serverpanel-init.sql
-- 或
--   mysql -uroot -p -e "source /path/to/serverpanel-init.sql"
--
-- 说明：
--   1. 业务账号/密码/授权为默认值，生产请修改下方 `SET @...` 变量后再执行；
--   2. 表结构共 16 张，InnoDB + utf8mb4，主键 BIGINT（雪花ID 应用侧生成）；
--   3. 种子数据含：管理员账号 admin / Admin@123、超管角色、全部菜单与按钮权限、
--      内置参数与字典。登录后请立即修改密码。
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 0. 配置区（按需修改）
--    数据库名、业务账号、密码均为默认值，生产环境请修改为强密码后执行。
--    注意：MySQL 的 CREATE USER / GRANT 不支持变量标识符，请直接改动下方字面量。
-- -----------------------------------------------------------------------------
-- 业务库名：server_panel
-- 业务账号：panel@'localhost'，密码：Panel@123456
-- 若需允许远程连接，将 'localhost' 改为 '%' 或指定网段。
...
-- 2. 创建业务账号并授权
-- -----------------------------------------------------------------------------
-- 若账号已存在，可先执行：DROP USER 'panel'@'localhost';
-- （Flyway 需要 performance_schema 的只读权限，否则迁移报 SELECT command denied）
CREATE USER IF NOT EXISTS 'panel'@'localhost' IDENTIFIED BY 'Panel@123456';
ALTER USER 'panel'@'localhost' IDENTIFIED BY 'Panel@123456';
...
-- 5. 完成
-- =============================================================================
SELECT 'ServerPanel 数据库初始化完成' AS result,
       COUNT(*) AS table_count FROM information_schema.tables WHERE table_schema = 'server_panel';
