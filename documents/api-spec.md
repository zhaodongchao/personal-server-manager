...
| 7xxx | 日常工具 | `7001` 文本过长（单次上限 1 MiB 字符）、`7002` 不支持的算法或编码方式、`7003` 密钥或 IV 不合法、`7004` 加解密失败、`7005` 输入不是合法的 Base64 / Hex、`7006` 混淆参数不合法、`7007` 不支持的混淆方式、`7008` 反混淆失败、`7009` JWT 格式非法、`7010` JWT Header 不合法、`7011` Payload 不是 JSON 对象、`7012` 不支持的 JWT 算法、`7013` JWT 密钥不合法、`7014` JWT 签名失败 |
...
菜单：一级「日常工具」(`/tools`) → 字符串加密解密 / URL 加密解密 / 图片与 Base64 互转 / 混淆加密解密 / JWT 工具。
...
> 审计口径同 §4：`@Audit(recordParams = false)` —— 请求体必然带密钥与 token，绝不落库；
