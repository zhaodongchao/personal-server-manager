...
| 7xxx | 日常工具 | `7001` 文本过长（单次上限 1 MiB 字符）、`7002` 不支持的算法或编码方式、`7003` 密钥或 IV 不合法、`7004` 加解密失败、`7005` 输入不是合法的 Base64 / Hex、`7006` 混淆参数不合法、`7007` 不支持的混淆方式、`7008` 反混淆失败 |
...
菜单：一级「日常工具」(`/tools`) → 字符串加密解密 / URL 加密解密 / 图片与 Base64 互转 / 混淆加密解密。
