...
> 定位：**纯计算**。不触碰宿主任何资源（不落盘、不调 hostagent、不访问数据库），
> 因此 `server-tools` 只依赖 `server-framework`，不依赖任何业务模块。

菜单：一级「日常工具」(`/tools`) → 字符串加密解密 / URL 加密解密 / 图片与 Base64 互转 / 混淆加密解密 / JWT 工具 / ID 生成器 / **二维码工具**。
其中 **URL 编解码与图片 Base64 互转为纯前端计算**，没有后端接口（也就没有审计面）。

### 1. 字符串加解密

| 方法 | 路径 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
| GET | `/crypto/options` | `tools:crypto:list` | 算法清单（摘要/HMAC/编码/对称算法及其形态约束），**界面按此渲染，不硬编码算法名** |
| POST | `/crypto/digest` | `tools:crypto:exec` | 摘要：`MD5` / `SHA1` / `SHA256` / `SHA512`，`upper` 控制大小写 |
| POST | `/crypto/hmac` | `tools:crypto:exec` | HMAC，密钥支持 `TEXT` / `HEX` / `BASE64` 三种录入方式 |
| POST | `/crypto/codec` | `tools:crypto:exec` | Base64 / Hex 编解码（`op` = `ENCODE` / `DECODE`） |
| POST | `/crypto/symmetric` | `tools:crypto:exec` | 对称加解密：`AES` / `DES` / `3DES` / `RC4` |

对称加解密要点：

- 方向语义 —— `ENCRYPT`：`text` 是明文，输出按 `outputEncoding` 编码；
  `DECRYPT`：`text` 是按 `inputEncoding` 编码的密文，输出明文。
- `AES` 密钥 16/24/32 字节、`CBC` 需 16 字节 IV；`DES` 8 字节 / IV 8；
  `3DES` 24 字节 / IV 8；`RC4` 为流算法，密钥任意长度、无分组模式与 IV。
- `DES` / `3DES` / `RC4` 均属已不安全的算法，界面带 `note` 明示「仅兼容旧系统」。

### 2. 可逆混淆

| 方法 | 路径 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
| GET | `/obfuscate/options` | `tools:obfuscate:list` | 混淆方式 + 各自的字段定义（`fields[]`） |
| POST | `/obfuscate/transform` | `tools:obfuscate:exec` | 混淆 / 反混淆，`params` 为按方式而定的键值 |

支持 `XOR`、`CAESAR`（凯撒 / ROT-N）、`REVERSE`、`BASE64_NESTED`、`ZERO_WIDTH`（零宽字符隐写）。

> **混淆不是加密**：以上方式全部可逆且不具备任何安全性，只能让明文不直接可读。
> 页面顶部有同样的警示文案。
...
| 方法 | 路径 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
| GET | `/tools/jwt/options` | `tools:jwt:list` | 算法白名单（HS / RS / PS / ES / EdDSA）与密钥录入方式 |
| POST | `/tools/jwt/verify` | `tools:jwt:exec` | 验证签名，返回 `verified` + 中文原因 |
| POST | `/tools/jwt/sign` | `tools:jwt:exec` | 签发 JWT（标注 `risky`，审计页可单独筛出） |

**验证不通过不是错误**：`/verify` 在密钥与签名不匹配时仍返回 `code=0`，
由 `data.verified=false` + `data.reason` 表达；只有输入本身非法（段数不对、
Header 不是 JSON、密钥解析失败）才返回 7xxx。

职责切分 —— **解码在前端，验签与签发在后端**：

- 前端（不需要密钥、不调后端）：Base64URL 解出 Header / Payload、声明含义说明、
  `exp` / `nbf` / `iat` 的时间校验与「已过期 / 尚未生效」提示；
- 后端（必须持有密钥）：验证签名、签发 token。

密钥录入方式（`keyFormat`）：

- `SECRET` —— 对称密钥文本，`secretEncoding` 支持 `TEXT` / `BASE64` / `HEX`；
- `PEM` —— 验签填公钥（`-----BEGIN PUBLIC KEY-----`），签发填 PKCS#8 私钥
  （`-----BEGIN PRIVATE KEY-----`）。PKCS#1 公钥（`BEGIN RSA PUBLIC KEY`）会自动包装成
  SubjectPublicKeyInfo；PKCS#1 私钥（`BEGIN RSA PRIVATE KEY`）不受支持，需先执行
  `openssl pkcs8 -topk8 -nocrypt`；
- `JWK` —— 单个 JWK 或 JWKS（`{"keys":[...]}`）；JWKS 按 token 的 `header.kid` 匹配，
  匹配不到则取第一个。**不做 JWKS URL 远程拉取**，避免服务端 SSRF。

算法与密钥类型（`keyType`）必须匹配，否则直接判不通过：

| 算法 | 族 | 密钥类型 | 说明 |
| ---- | ---- | ---- | ---- |
| `HS256` / `HS384` / `HS512` | HMAC | HMAC | 对称密钥，任意长度字节串 |
| `RS256` / `RS384` / `RS512` | RSA | RSA | RSASSA-PKCS1-v1_5 |
| `PS256` / `PS384` / `PS512` | RSA-PSS | RSA | 盐长度 32 / 48 / 64 |
| `ES256` / `ES384` / `ES512` | ECDSA | EC | 曲线必须为 P-256 / P-384 / P-521 |
| `EdDSA` | EdDSA | OKP | Ed25519（RFC 8037） |

实现要点：

- 算法走白名单映射，用户传入的字符串只作查表键，**不拼进 `Signature.getInstance()`**；
- **算法混淆防护**：`header.alg` 所需密钥类型必须与实际提供的密钥一致，否则判不通过
  —— 阻断「把 RSA 公钥当作 HS256 密钥」这类经典绕过；
- `alg=none` 恒判不通过，且不提供 none 的签发能力；
- HMAC 使用 `MessageDigest.isEqual` 做常量时间比较；
- ECDSA 在 JOSE（`r||s` 定长）与 JDK（DER）之间做双向转换。

> 审计口径同 §6：`@Audit(recordParams = false)` —— 请求体必然带密钥与 token，绝不落库；
> `/sign` 额外标注 `risky = true`。
...
| 大类 | 方案 | 位段（高位→低位） | 生成方 | 有序 |
| ---- | ---- | ---- | ---- | ---- |
| `DB` 数据库原生自增 | `MYSQL_AUTO_INCREMENT` | 无位段（表级计数器） | 数据库 | 是 |
| | `SEQUENCE` | 无位段（独立序列对象） | 数据库 | 是 |
| `RANDOM` 随机 / 时间哈希 | `UUID_V1` | `32+16+4+12+2+14+48` | 应用 | 是 |
| | `UUID_V4` | `32+16+4+12+2+62` | 应用 | 否 |
| | `UUID_V7` | `48+4+12+2+62` | 应用 | 是 |
| | `OBJECT_ID` | `32+40+24` | 驱动 | 是 |
| `SNOWFLAKE` 雪花及变种 | `SNOWFLAKE` | `1+41+10+12` | 应用 | 是 |
...
- `machineId` / `workerId` / `datacenterId` / `sequence` 等由 `params[]` 下发，留空按随机值取；
- 时钟回拨用 `clockBackwardMs` 模拟，`backwardStrategy` 选策略：
  `WAIT`（等物理时间追平逻辑时间）/ `REJECT`（直接拒绝，错误码 `7019`）/ `TOLERATE`（沿用逻辑时间戳）。
  **本页不会真的阻塞线程**，等待耗时只写进 `notes[]`。

时间基准（`epoch`）决定 ID 的可用年限，是本页重点科普内容：

| 方案 | 默认基准（UTC） | 时间位宽 | 可用年限 |
| ---- | ---- | ---- | ---- |
| `SNOWFLAKE` | `1288834974657`（2010-11-04 09:42:54.657） | 41 位毫秒 | 约 69.7 年 |
| `UID_GENERATOR` | `1767225600000`（2026-01-01 00:00:00） | 28 位秒 | 约 8.51 年 |
| `SONYFLAKE` | `1409529600000`（2014-09-01 00:00:00） | 39 位 × 10ms | 约 174 年 |

> **百度原始基准已经用尽**：`1463673600000`（2016-05-20）配 28 位秒级时间戳，
> `2^28` 秒 ≈ 8.51 年，2024 年底前后就耗尽了。选中该基准会直接返回 `7017` 并附完整解释，
> 本页默认改用 `2026-01-01`。

反解的 `epoch` 参数：雪花类的时间戳是「相对基准的偏移」，反解必须知道当初用的基准，
因此 `/tools/id/decode` 接受 `epoch`（**留空则用上表的默认基准**）。

**反解不通过不是错误**（与 JWT `/verify` 同口径）：值可解析但与结构不符时
（版本位不对、变体位不是 `10`、时间偏移超出位宽）返回 `code=0` + `data.valid=false` +
`data.reason` 中文诊断 —— 页面把它当**结论**展示，而不是弹一个错误提示，
这样用户能直接看到「版本位是 100（4），而 UUIDv1 的版本位固定为 1」这类具体原因。
只有**超长**才返回 `7018`；**空值**由请求体参数校验（`@NotBlank`）拦下，
返回全站通用的校验错误码 `400`（同样 HTTP 200 + `body.code`）。

实现要点：

- `UUID_V1` **不读服务器真实网卡** —— 默认生成随机 node 并把组播位置 1（RFC 4122 §4.5），
  避免泄露硬件地址，页面对此有提示；
- `UUID_V4` / `UUID_V7` 的版本位与变体位由生成与反解双侧校验（不符则 `valid=false` + `reason`）；
- 批量结果的**逐条位段拆解上限 100 条**（`segLimit`），超出则剥离 `segValues`
  并置 `segValuesIncluded=false` + `notes[]` 说明，控制响应体大小；
- 数字型 ID 由前端以**字符串**接收（雪花 64 位 Long 超出 JS `Number.MAX_SAFE_INTEGER`），
  CSV 导出带 BOM，Excel 打开不乱码。

> 审计口径见 §6：本页**保留入参记录**（`@Audit` 默认 `recordParams = true`），
> 因为入参只有方案名、数量、机器号，不含任何凭据；能回答「谁批量生成过什么方案」。

### 5. 二维码工具 `/tools/qrcode`

| 方法 | 路径 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
| GET | `/tools/qrcode/options` | `tools:qrcode:list` | 内容类型（含各自的 `fields[]` 与 `notice`）、容错等级、码点样式与各项上限 |
| POST | `/tools/qrcode/generate` | `tools:qrcode:exec` | 生成二维码，返回 data URL 图片与真实尺寸 |
| POST | `/tools/qrcode/decode` | `tools:qrcode:exec` | 识别图片中的二维码（识别不出不是错误） |

**三条必须在界面上如实说明的事实**：

1. **二维码里只有文本**。所谓「WiFi 码 / 名片码 / 付款码」只是扫码端按约定前缀解析同一串
   文本（`WIFI:` / `BEGIN:VCARD` / `bitcoin:` …），码本身没有类型之分；拼装规则全部在服务端。
2. **小程序码（菊花朵码）与公众号带场景值二维码不归本工具覆盖**。它们必须由微信服务端
   API（appid + secret 换 access_token 后调用 `wxacode.get` / `qrcode/create`）生成，
   任何第三方都无法离线算出。`WECHAT_MINIAPP` 产出的是「内容为小程序路径或链接的通用二维码」，
   该类型的 `notice` 里写明了这一点，前端以 Alert 展示。
3. **边长会吸附到模块的整数倍**。请求 333px 而实际输出 315px（45 模块 × 7px）是刻意行为：
   非整数倍缩放会让相邻码点糊在一起，密集内容直接扫不出来。返回值同时给出
   `requestSize` / `realSize` / `scale` / `moduleCount`，界面如实展示差异。

样式控制：`size`（64~2048）、`margin`（静默区模块数）、`ecc`（L/M/Q/H）、
`fgColor`/`bgColor`/`gradientColor`、`dotStyle`（SQUARE/DOT/ROUNDED）、
`logoBase64` + `logoScale`（上限 25%）+ `logoRound`、`format`（PNG/JPEG）。

- **嵌 Logo 必须留白边**：直接把 Logo 压在码点上会让贴边那一圈模块与 Logo 混成一片，
  这是「加了 Logo 就扫不出来」的头号原因；服务端自动描一圈背景色，并建议把 `ecc` 调到 `H`。
- **圆点样式会被解码器判死**：正圆相切而不重叠时，反锯齿把接缝吃掉，实测 ZXing 直接识别不出。
  因此圆点刻意按模块宽的 12% 外扩让相邻点重叠，圆角半径压到 1/4 —— 三种样式均已用
  「生成 → 再识别」回归验证。
- **识别不出不是错误**（与 JWT `/verify`、ID `/decode` 同口径）：返回 `code=0` +
  `data.found=false` + `data.reason`；只有图片本身非法（非图片 / base64 损坏 / 超限）才返回 7xxx。

> 审计口径见 §6：`@Audit(recordParams = false)` —— 请求体里可能有 WiFi 明文口令、
> 名片手机号、收款地址与整张 base64 图片，这些都不得落库。

### 6. 审计口径（重要）

这些接口处理的往往是口令、Token、密钥，因此全部标注 `@Audit(recordParams = false)`：
保留「谁、在什么时候、用了哪个能力、结果如何」的留痕（`module=tools`），
但 `sys_audit_log.params` 写入占位符 `[按端点配置不记录入参]`，**入参整体不落库**。

> **例外**：ID 生成器（§4）不适用本条 —— 它的入参不含凭据，因此保留默认的 `recordParams = true`，入参照常落库。

不能依赖既有脱敏：脱敏只匹配键名含 `password`/`secret`/`token` 等的字段，
而工具接口的入参键名是 `text` / `key`，不在敏感词表里，脱敏根本不会命中。

## 九、前端接入要点（Vben Admin 5）
