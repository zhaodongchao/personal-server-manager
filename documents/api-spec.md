...
菜单：一级「日常工具」(`/tools`) → 字符串加密解密 / URL 加密解密 / 图片与 Base64 互转 / 混淆加密解密 / JWT 工具 / ID 生成器。
...
> 审计口径同 §5：`@Audit(recordParams = false)` —— 请求体必然带密钥与 token，绝不落库；
...
> 审计口径见 §5：本页**保留入参记录**（`@Audit` 默认 `recordParams = true`），
